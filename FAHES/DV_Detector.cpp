...
void DV_Detector::detect_single_char_strings(TableProfile TP, 
    			std::vector<sus_disguised> & sus_dis_values,char * column){
...
if (TP.header[i].compare(column) != 0) {
		continue;
		}
...
void DV_Detector::check_non_conforming_patterns(TableProfile & TP, 
	...
	detect_single_char_strings(TP, sus_dis_values, column);
	// cerr << "Done : detect_single_char_strings\n"; 
	positive_negative_inconsistency(TP, sus_dis_values, column);
	// cerr << "Done : positive_negative_inconsistency\n";
	check_repeated_substrings(TP, M, sus_dis_values,column);
